...
CREATE OR REPLACE ROLE PSUBASH_HOL_ROLE;
GRANT ROLE PSUBASH_HOL_ROLE TO ROLE SYSADMIN;
GRANT ROLE PSUBASH_HOL_ROLE TO USER IDENTIFIER($MY_USER);

GRANT EXECUTE TASK ON ACCOUNT TO ROLE PSUBASH_HOL_ROLE;
GRANT MONITOR EXECUTION ON ACCOUNT TO ROLE PSUBASH_HOL_ROLE;
GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE PSUBASH_HOL_ROLE;

-- Databases
CREATE OR REPLACE DATABASE PSUBASH_HOL_DB;
GRANT OWNERSHIP ON DATABASE PSUBASH_HOL_DB TO ROLE PSUBASH_HOL_ROLE;

-- Warehouses
CREATE OR REPLACE WAREHOUSE HOL_WH WAREHOUSE_SIZE = XSMALL, AUTO_SUSPEND = 300, AUTO_RESUME= TRUE;
GRANT OWNERSHIP ON WAREHOUSE HOL_WH TO ROLE PSUBASH_HOL_ROLE;
